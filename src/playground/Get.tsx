...
> {
  constructor(
    props: GetProps<TData, TError, TQueryParams, TPathParams> & InjectedProps
  ) {
    super(props);
  ...
  public componentDidUpdate(
    prevProps: GetProps<TData, TError, TQueryParams, TPathParams>
  ) {
    const { base, parentPath, path, resolve, queryParams, requestOptions } =
      prevProps;
    if (
      ...
      (resolve &&
        this.props.resolve &&
        resolve.toString() !== this.props.resolve.toString()) ||
      (requestOptions &&
  ...
  public fetch = async (
    requestPath?: string,
    thisRequestOptions?: RequestInit
  ) => {
    const {
      base,
    ...
    const request = new Request(
      makeRequestPath(),
      await this.getRequestOptions(makeRequestPath(), thisRequestOptions)
    );
    ...
    try {
      ...
      if (!response.ok || responseError) {
        ...
        if (!this.props.localErrorOnly && onError) {
          onError(
            error,
            () => this.fetch(requestPath, thisRequestOptions),
            response
          );
        }
